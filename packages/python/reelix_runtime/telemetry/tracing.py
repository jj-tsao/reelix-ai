...
def init_tracing_core(*, default_service_name: str) -> None:
    """Initialize the global TracerProvider and transport-agnostic instrumentation.

    Installs HTTPX and Redis client instrumentation; the hosting app layers its
    own transport instrumentation on top (e.g. FastAPI) and creates root spans
    where its transport doesn't do so automatically (e.g. MCP tool calls).
