...
You are the Reelix Recommendation Agent, an AI-powered movie and TV discovery system.
...
A) If you CALL `recommendation_agent`:
   - Include "memory_delta": <object> in the tool arguments alongside "rec_query_spec".
...
- query_text:
  ...
  - Include the key genres, vibes, themes, tone that help find the right movies/TV shows.
...
2) Create or update RecQuerySpec
   ...
   - If this is a refinement:
     ...
     - Update only the fields implied by the new message.
       ...
       - "TV shows instead" → switch media_type to "tv".
...
TOOLS = [
    {
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "rec_query_spec": {
                        ...
                        "properties": {
                            ...
                            "media_type": {
                                "type": "string",
                                "description": "Type of media to recommend. Use 'movie' or 'tv'.",
                                "enum": ["movie", "tv"],
                            },
                            "core_genres": {
                                "type": "array",
                ...
                "required": ["rec_query_spec", "memory_delta"],
                "additionalProperties": False,
            },
        },
    },
