...
import { signInWithPassword, getCurrentSession, getAppUser, sendMagicLink, signInWithOAuth } from "../api";
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      if (!usePassword) {
        ...
        toast({ title: "Check your email", description: "We sent a sign-in link. Open it to continue.", variant: "success", duration: null });
        return;
      }
      ...
      <section className="mx-auto w-full max-w-md">
        <form onSubmit={onSubmit} className="space-y-4 rounded-lg border p-4">
          ...
          <Button type="submit" disabled={submitting} className="w-full">
            {submitting ? "Please wait…" : usePassword ? "Sign In" : "Email me a sign-in link"}
          </Button>
          {!usePassword && (
            <p className="text-xs text-muted-foreground text-center">No password needed.</p>
          )}
