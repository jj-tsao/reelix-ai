export const TEASER_TITLES = {
  smart_thoughtful: [
    {
      title: "Parasite",
      release_year: 2019,
      media_id: 496243,
      genres: ["Comedy", "Thriller", "Drama"],
      poster_url:
        "https://image.tmdb.org/t/p/w500/7IiTTgloJzvGI1TAYymCfbfl3vT.jpg",
      backdrop_url:
        "https://image.tmdb.org/t/p/w1280/hiKmpZMGZsrkA3cdce8a7Dpos1j.jpg",
      trailer_key: "bM9QabAojCg",
      imdb_rating: 8.5,
      rotten_tomatoes_rating: 99,
      why_you_might_enjoy_it:
        "With its razor-sharp social satire and escalating tension, Parasite aligns well with your taste for psychologically layered, character-driven dramas. Its genre-bending turns explore class, morality, and family dynamics in ways similar to the uneasy social critique you enjoy in Nightcrawler and Gone Girl.",
    },

    {
      title: "Everything Everywhere All at Once",
      release_year: 2022,
      media_id: 545611,
      genres: ["Action", "Adventure", "Science Fiction"],
      poster_url:
        "https://image.tmdb.org/t/p/w500/u68AjlvlutfEIcpmbYpKcdi09ut.jpg",
      backdrop_url:
        "https://image.tmdb.org/t/p/w1280/ss0Os3uWJfQAENILHZUdX8Tt1OC.jpg",
      trailer_key: "wxN1T1uxQ2g",
      imdb_rating: 7.8,
      rotten_tomatoes_rating: 93,
      why_you_might_enjoy_it:
        "Everything Everywhere All at Once aligns well with your taste for character-driven stories with psychological depth, blending surreal multiverse play with intimate family stakes. Its mix of humor and existential reflection echoes the inventive emotion you appreciate in Her and Eternal Sunshine of the Spotless Mind.",
    },

    {
      title: "Interstellar",
      release_year: 2014,
      media_id: 157336,
      genres: ["Adventure", "Drama", "Science Fiction"],
      poster_url:
        "https://image.tmdb.org/t/p/w500/gEU2QniE6E77NI6lCU6MxlNBvIx.jpg",
      backdrop_url:
        "https://image.tmdb.org/t/p/w1280/ln2Gre4IYRhpjuGVybbtaF4CLo5.jpg",
      trailer_key: "LY19rHKAaAg",
      imdb_rating: 8.7,
      rotten_tomatoes_rating: 73,
      why_you_might_enjoy_it:
        "Interstellar aligns with your interest in idea-driven sci-fi anchored by human stakes. Its time-bending narrative and contemplative tone pair cosmic spectacle with emotional resonance, similar to the reflective depth you appreciate in Arrival and Gravity.",
    },

    {
      title: "Ex Machina",
      release_year: 2015,
      media_id: 264660,
      genres: ["Drama", "Science Fiction"],
      poster_url:
        "https://image.tmdb.org/t/p/w500/dmJW8IAKHKxFNiUnoDR7JfsK7Rp.jpg",
      backdrop_url:
        "https://image.tmdb.org/t/p/w1280/uqOuJ50EtTj7kkDIXP8LCg7G45D.jpg",
      trailer_key: "hEcB7T-C0g8",
      imdb_rating: 7.7,
      rotten_tomatoes_rating: 92,
      why_you_might_enjoy_it:
        "With its minimalist tension and questions of power, trust, and AI, Ex Machina aligns well with your preference for cerebral sci-fi over pure spectacle. Its precise pacing and ethical ambiguity mirror the cool futurism you appreciate in Black Mirror and Blade Runner 2049.",
    },

    {
      title: "The Prestige",
      release_year: 2006,
      media_id: 1124,
      genres: ["Drama", "Mystery", "Science Fiction"],
      poster_url:
        "https://image.tmdb.org/t/p/w500/bdN3gXuIZYaJP7ftKK2sU0nPtEA.jpg",
      backdrop_url:
        "https://image.tmdb.org/t/p/w1280/nlqYna8K075J2j5E13Rs1Gzq2HJ.jpg",
      trailer_key: "ijXruSzfGEc",
      imdb_rating: 8.5,
      rotten_tomatoes_rating: 77,
      why_you_might_enjoy_it:
        "The Prestige aligns with your taste for twist-rich mysteries and meticulous character studies. Its obsession-driven rivalry and layered reveals reward patient viewers, similar to the puzzle-box intrigue of Shutter Island and the craft-obsession of The Social Network.",
    },

    {
      title: "Pulp Fiction",
      release_year: 1994,
      media_id: 680,
      genres: ["Thriller", "Crime", "Comedy", "Drama"],
      poster_url:
        "https://image.tmdb.org/t/p/w500/vQWk5YBFWF4bZaofAbv0tShwBvQ.jpg",
      backdrop_url:
        "https://image.tmdb.org/t/p/w1280/suaEOtk1N1sgg2MTM7oZd2cfVp3.jpg",
      trailer_key: "tGpTpVyI_OQ",
      imdb_rating: 8.9,
      rotten_tomatoes_rating: 92,
      why_you_might_enjoy_it:
        "Pulp Fiction’s satirical and quirky storytelling, combined with its nonlinear narrative, aligns well with your interest in character-driven films like Everything Everywhere All at Once. Its dark humor and intense situations reflect the psychological and suspenseful elements you appreciate.",
    },
    {
      title: "Blade Runner 2049",
      release_year: 2017,
      media_id: 335984,
      genres: ["Science Fiction", "Drama"],
      poster_url:
        "https://image.tmdb.org/t/p/w500/gajva2L0rPYkEWjzgFlBXCAVBE5.jpg",
      backdrop_url:
        "https://image.tmdb.org/t/p/w1280/iqZWmj1Ifz4T5tuCuszWktUG5yX.jpg",
      trailer_key: "gCcx85zbxz4",
      imdb_rating: 8,
      rotten_tomatoes_rating: 88,
      why_you_might_enjoy_it:
        "With its lush world-building and existential questions, Blade Runner 2049 aligns with your taste for atmospheric, philosophically inclined sci-fi. Its meditative noir mood and identity themes echo the films you like, such as Dune and Memento.",
    },
  ],

  "comfort_feel-good": [
    {
      title: "Paddington 2",
      release_year: 2017,
      media_id: 346648,
      genres: ["Adventure", "Comedy", "Family"],
      poster_url:
        "https://image.tmdb.org/t/p/w500/1OJ9vkD5xPt3skC6KguyXAgagRZ.jpg",
      backdrop_url:
        "https://image.tmdb.org/t/p/w1280/AmoYMCH3rz7r4b7QMZfcOprV7Uh.jpg",
      trailer_key: "52x5HJ9H8DM",
      imdb_rating: 7.8,
      rotten_tomatoes_rating: 99,
      why_you_might_enjoy_it:
        "Paddington 2 aligns well with your taste for kind, uplifting stories that land humor without cynicism. Its gentle warmth and visual charm create a true comfort watch, similar to the cozy spirit of The Grand Budapest Hotel and Chef.",
    },

    {
      title: "Begin Again",
      release_year: 2014,
      media_id: 198277,
      genres: ["Comedy", "Music", "Romance", "Drama"],
      poster_url:
        "https://image.tmdb.org/t/p/w500/qx4HXHXt528hS4rwePZbZo20xqZ.jpg",
      backdrop_url:
        "https://image.tmdb.org/t/p/w1280/caEE4HnHdIkofsYgDwQSflfuny8.jpg",
      trailer_key: "uTRCxOE7Xzc",
      imdb_rating: 7.4,
      rotten_tomatoes_rating: 83,
      why_you_might_enjoy_it:
        "With its heartfelt, music-driven storytelling and gentle humor, Begin Again aligns well with your taste for low-stakes, character-first comfort watches. Its focus on creativity, connection, and second chances offers a quietly uplifting vibe, similar to the warmth of Chef and the musical optimism of La La Land.",
    },

    {
      title: "Lady Bird",
      release_year: 2017,
      media_id: 391713,
      genres: ["Drama", "Comedy"],
      poster_url:
        "https://image.tmdb.org/t/p/w500/gl66K7zRdtNYGrxyS2YDUP5ASZd.jpg",
      backdrop_url:
        "https://image.tmdb.org/t/p/w1280/b6XrxL6iRfhRn4PrZ4JehrLpgdD.jpg",
      trailer_key: "cNi_HC839Wo",
      imdb_rating: 7.4,
      rotten_tomatoes_rating: 99,
      why_you_might_enjoy_it:
        "Lady Bird aligns well with your taste for intimate coming-of-age stories that balance humor with honesty. Its authentic voice and emotional specificity recall the tender realism of The Florida Project and Frances Ha.",
    },

    {
      title: "Amélie",
      release_year: 2001,
      media_id: 194,
      genres: ["Comedy", "Romance"],
      poster_url:
        "https://image.tmdb.org/t/p/w500/nSxDa3M9aMvGVLoItzWTepQ5h5d.jpg",
      backdrop_url:
        "https://image.tmdb.org/t/p/w1280/6n53UI4mX9QMfe2S0Pgt8mGebY1.jpg",
      trailer_key: "QY6Qt0ZUeVQ",
      imdb_rating: 8.3,
      rotten_tomatoes_rating: 90,
      why_you_might_enjoy_it:
        "With whimsical storytelling and playful visual language, Amélie aligns with your taste for tender, quirky romances. Its lighthearted charm and imaginative details feel similar to your favorites: Midnight in Paris and Before Sunrise.",
    },

    {
      title: "Her",
      release_year: 2013,
      media_id: 152601,
      genres: ["Romance", "Science Fiction", "Drama"],
      poster_url:
        "https://image.tmdb.org/t/p/w500/eCOtqtfvn7mxGl6nfmq4b1exJRc.jpg",
      backdrop_url:
        "https://image.tmdb.org/t/p/w1280/zxttikp0SNEtF4vWsg70Vs6McFi.jpg",
      trailer_key: "ne6p6MfLBxc",
      imdb_rating: 8,
      rotten_tomatoes_rating: 95,
      why_you_might_enjoy_it:
        "Her aligns with your interest in introspective romance that uses near-future tech to explore love and identity. Its gentle color palette and melancholic warmth evoke the reflective mood of films you enjoy - such as Lost in Translation and Eternal Sunshine of the Spotless Mind.",
    },

    {
      title: "La La Land",
      release_year: 2016,
      media_id: 313369,
      genres: ["Comedy", "Drama", "Romance", "Music"],
      poster_url:
        "https://image.tmdb.org/t/p/w500/uDO8zWDhfWwoFdKS4fzkUJt0Rf0.jpg",
      backdrop_url:
        "https://image.tmdb.org/t/p/w1280/nlPCdZlHtRNcF6C9hzUH4ebmV1w.jpg",
      trailer_key: "0pdqf4P9MB8",
      imdb_rating: 8,
      rotten_tomatoes_rating: 91,
      why_you_might_enjoy_it:
        "With bittersweet romance and modern musical verve, La La Land aligns well with your taste for ambitious, performance-driven stories. Its dreamy set-pieces and grounded emotion echo the optimism of Sing Street and the artistic drive of Whiplash you like.",
    },

    {
      title: "About Time",
      release_year: 2013,
      media_id: 122906,
      genres: ["Drama", "Romance", "Fantasy"],
      poster_url:
        "https://image.tmdb.org/t/p/w500/iR1bVfURbN7r1C46WHFbwCkVve.jpg",
      backdrop_url:
        "https://image.tmdb.org/t/p/w1280/fzZmcKQv7ZTGIiPvocPhNs3wUyK.jpg",
      trailer_key: "7OIFdWk83no",
      imdb_rating: 7.8,
      rotten_tomatoes_rating: 71,
      why_you_might_enjoy_it:
        "About Time aligns with your preference for heartfelt, quietly funny romances that value everyday kindness. Its time-twist premise supports a warm family story, similar to the gentle sincerity of films you like - Begin Again and The Notebook.",
    },
  ],

  kinetic_spectacle: [
    {
      title: "Mad Max: Fury Road",
      release_year: 2015,
      media_id: 76341,
      genres: ["Action", "Adventure", "Science Fiction"],
      poster_url:
        "https://image.tmdb.org/t/p/w500/hA2ple9q4qnwxp3hKVNhroipsir.jpg",
      backdrop_url:
        "https://image.tmdb.org/t/p/w1280/n6vWE0bXq4ImAM00iM5gWyORNSM.jpg",
      trailer_key: "MonFNCgK4WE",
      imdb_rating: 8.1,
      rotten_tomatoes_rating: 97,
      why_you_might_enjoy_it:
        "With precision action and tactile world-building, Mad Max: Fury Road aligns well with your taste for visually choreographed set-pieces. Its relentless momentum and surprising heart mirror the craftsmanship you enjoy in John Wick.",
    },

    {
      title: "Top Gun: Maverick",
      release_year: 2022,
      media_id: 361743,
      genres: ["Action", "Drama"],
      poster_url:
        "https://image.tmdb.org/t/p/w500/62HCnUTziyWcpDaBO2i1DX17ljH.jpg",
      backdrop_url:
        "https://image.tmdb.org/t/p/w1280/kBSSbN1sOiJtXjAGVZXxHJR9Kox.jpg",
      trailer_key: "giXco2jaZ_4",
      imdb_rating: 8.2,
      rotten_tomatoes_rating: 96,
      why_you_might_enjoy_it:
        "Top Gun: Maverick aligns with your interest in high-energy thrills that still carry earnest character stakes. Its practical aerial sequences and crowd-pleasing arc recall Mission: Impossible – Fallout and the uplifting pulse of Creed.",
    },

    {
      title: "The Lord of the Rings: The Return of the King",
      release_year: 2003,
      media_id: 122,
      genres: ["Adventure", "Fantasy", "Action"],
      poster_url:
        "https://image.tmdb.org/t/p/w500/rCzpDGLbOoPwLjy3OAm5NUPOTrC.jpg",
      backdrop_url:
        "https://image.tmdb.org/t/p/w1280/2u7zbn8EudG6kLlBzUYqP8RyFU4.jpg",
      trailer_key: "zckJCxYxn1g",
      imdb_rating: 9,
      rotten_tomatoes_rating: 94,
      why_you_might_enjoy_it:
        "With mythic scale and emotional payoff, The Return of the King aligns well with your taste for ensemble journeys and epic world-building. Its sweeping battles and friendship-first stakes evoke some of your favorites - Gladiator and Braveheart.",
    },

    {
      title: "Spider-Man: Into the Spider-Verse",
      release_year: 2018,
      media_id: 324857,
      genres: ["Animation", "Action", "Adventure", "Science Fiction"],
      poster_url:
        "https://image.tmdb.org/t/p/w500/iiZZdoQBEYBv6id8su7ImL0oCbD.jpg",
      backdrop_url:
        "https://image.tmdb.org/t/p/w1280/8mnXR9rey5uQ08rZAvzojKWbDQS.jpg",
      trailer_key: "tg52up16eq0",
      imdb_rating: 8.4,
      rotten_tomatoes_rating: 97,
      why_you_might_enjoy_it:
        "Spider-Man: Into the Spider-Verse aligns with your taste for stylish, music-laced hero stories that balance fun with heart. Its inventive animation and rhythmic storytelling feel similar to Scott Pilgrim and The Lego Movie that you like.",
    },

    {
      title: "Dune: Part Two",
      release_year: 2024,
      media_id: 693134,
      genres: ["Science Fiction", "Adventure"],
      poster_url:
        "https://image.tmdb.org/t/p/w500/1pdfLvkbY9ohJlCjQH2CZjjYVvJ.jpg",
      backdrop_url:
        "https://image.tmdb.org/t/p/w1280/f6ahdeUZPwUVirr86gcZ1QmiasQ.jpg",
      trailer_key: "U2Qp5pL3ovA",
      imdb_rating: 8.4,
      rotten_tomatoes_rating: 92,
      why_you_might_enjoy_it:
        "With operatic stakes and immersive design, Dune: Part Two aligns well with your interest in slow-burn, world-driven sci-fi. Its atmosphere and thematic sweep echo some of your favorite films: Blade Runner 2049 and Interstellar.",
    },

    {
      title: "Casino Royale",
      release_year: 2006,
      media_id: 36557,
      genres: ["Adventure", "Action", "Thriller"],
      poster_url:
        "https://image.tmdb.org/t/p/w500/lMrxYKKhd4lqRzwUHAy5gcx9PSO.jpg",
      backdrop_url:
        "https://image.tmdb.org/t/p/w1280/4sO09d1zReaSl3F08choHVZ9p8r.jpg",
      trailer_key: "U4NT78c-pYs",
      imdb_rating: 8,
      rotten_tomatoes_rating: 94,
      why_you_might_enjoy_it:
        "Casino Royale aligns with your taste for grounded spy action where character stakes matter. Its sleek set-pieces and darker edge recall The Bourne Ultimatum and the crime-drama intensity of Heat.",
    },

    {
      title: "Spirited Away",
      release_year: 2001,
      media_id: 129,
      genres: ["Animation", "Family", "Fantasy"],
      poster_url:
        "https://image.tmdb.org/t/p/w500/39wmItIWsg5sZMyRUHLkWBcuVCM.jpg",
      backdrop_url:
        "https://image.tmdb.org/t/p/w1280/ukfI9QkU1aIhOhKXYWE9n3z1mFR.jpg",
      trailer_key: "GAp2_0JJskk",
      imdb_rating: 8.6,
      rotten_tomatoes_rating: 96,
      why_you_might_enjoy_it:
        "With hand-crafted wonder and gentle adventure, Spirited Away aligns well with your taste for lyrical, transportive animation. Its cozy awe and emotional clarity are similar to films you enjoy, such as My Neighbor Totoro and Kiki’s Delivery Service.",
    },

    {
      title: "1917",
      release_year: 2019,
      media_id: 530915,
      genres: ["War", "History", "Thriller"],
      poster_url:
        "https://image.tmdb.org/t/p/w500/iZf0KyrE25z1sage4SYFLCCrMi9.jpg",
      backdrop_url:
        "https://image.tmdb.org/t/p/w1280/2lBOQK06tltt8SQaswgb8d657Mv.jpg",
      trailer_key: "gZjQROMAh_s",
      imdb_rating: 8.2,
      rotten_tomatoes_rating: 88,
      why_you_might_enjoy_it:
        "With its bold “one-shot” presentation, 1917 aligns with your taste for technical showpieces that stay character-first. The real-time urgency turns historical stakes into immediate, visceral momentum.",
    },

    {
      title: "The Revenant",
      release_year: 2015,
      media_id: 281957,
      genres: ["Western", "Drama", "Adventure"],
      poster_url:
        "https://image.tmdb.org/t/p/w500/ji3ecJphATlVgWNY0B0RVXZizdf.jpg",
      backdrop_url:
        "https://image.tmdb.org/t/p/w1280/sGOw65ZmfgGfistCjuNsazjs6j9.jpg",
      trailer_key: "LoebZZ8K5N0",
      imdb_rating: 8,
      rotten_tomatoes_rating: 78,
      why_you_might_enjoy_it:
        "With immersive survival storytelling and natural-light cinematography, The Revenant aligns with your taste for visceral, craft-forward epics. Its long takes and elemental stakes deliver raw immediacy, similar to the real-time tension you enjoy in 1917.",
    },
  ],
};
